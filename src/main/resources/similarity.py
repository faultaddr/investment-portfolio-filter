...
def picture(index, users, production, history):
    ...
    plt.axis('equal')
    plt.savefig('E:\code\invest\src\main\\resources\static\Gender.jpg')
    plt.close()
    # Family Members
    selected[selected.columns[3]].plot.hist(title='Family Members',color='deepskyblue')
    plt.savefig('E:\code\invest\src\main\\resources\static\FamilyMembers.jpg')
    plt.close()
    # Age
    selected[selected.columns[1]].plot.hist(title='Age', color='mediumpurple')
    plt.savefig('E:\code\invest\src\main\\resources\static\Age.jpg')
    plt.close()
    # WorkTime
    selected[selected.columns[7]].plot.hist(title='Work Time')
    plt.savefig('E:\code\invest\src\main\\resources\static\WorkTime.jpg')
    plt.close()
    # Income
    selected[selected.columns[11]].plot.hist(title='Income',color='gold',bins=15)
    plt.savefig('E:\code\invest\src\main\\resources\static\Income.jpg')
    plt.close()
    # Production
    buy_set = set()
    for i in index:
    ...
    plt.savefig("E:\code\invest\src\main\\resources\static\Productions.jpg")
    plt.close()
